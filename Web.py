...
st.set_page_config(layout='wide',page_title='CRIS-EC',page_icon='./logo.png')

model = pickle.load(open("./model.save", "rb"))

st.markdown('# <center>CRIS-EC for Endometrial Cancer</center>',unsafe_allow_html=True)
st.write('')
...
st.line_chart(df,x='Survival Time/day',y='Survival Probability',color='#7E60BF')
...
with col22:
    if risk_score >= 9.617087:
        t4 = st.text_input(label='**Recommended Treatment Strategies**',value=str('BI 2536, Bortezomib, Ibrutinib, and MK-1775'))
    else:
        t4 = st.text_input(label='**Recommended Treatment Strategies**', value=str('ICB Therapy, Cisplatin, Docetaxel, Paclitaxel, and Topotecan'))